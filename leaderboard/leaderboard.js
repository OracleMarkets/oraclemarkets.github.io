const AVATAR_API = "https://render.crafty.gg/3d/bust/";
...
function loadMembers() {
    ...
    members = data.entries.slice(0, MAX_ENTRIES).map((entry, index) => ({
        ...entry,
        position: index + 1,
...
function avatarUrl(entry) {
    const id = entry.skinId || entry.name;
...
function renderMembers(filteredMembers = members) {
    ...
    filteredMembers.forEach((m, i) => {
        const tier = m.tier;
        const imageUrl = avatarUrl(m);
        const affiliation = m.nation || m.guild
            ? `<div class="player-affiliation">
                    ${m.nation ? `<span class="origin">${escapeHtml(m.nation)}</span>` : ""}
                    ${m.guild ? `<span class="region">${escapeHtml(m.guild)}</span>` : ""}
               </div>`
            : "";

        const card = document.createElement("div");
        card.className = `rank-card ${tier}`;
        card.style.setProperty("--rank-color", `var(--${tier})`);
        card.innerHTML = `
            <div class="rank-inner" style="animation-delay:${i * 0.04}s">
                <div class="rank-header">
                    <h1 class="rank-number">#${m.position}</h1>
                    <img src="${imageUrl}" alt="" width="92" height="92" loading="lazy" decoding="async">
                </div>
                <div class="mage-info">
